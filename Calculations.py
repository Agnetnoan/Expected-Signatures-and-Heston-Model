...
# Define the ranges of Heston parameters
thetas=np.arange(0.1,1,0.2)
kappas = np.arange(3,4,0.2)
v_0s = thetas #  = reversion_level
rhos = np.arange(0.1,1,0.25)
sigmas = np.arange(0.1,1,0.25)
rs = np.arange(0.01,0.10,0.02)
                ...
                for kappa in kappas:
                    for theta in thetas:
                        ...
                        for j in range(paths):
                            prices_and_sigs_with_time_tensor[j] = torch.tensor([prices[j], sigs[j], time_array]).T
                        sign_ps_signatory_ranges[count] = signatory.signature(prices_and_sigs_with_time_tensor, 2)
                        count = count + 1
    return sign_ps_signatory_ranges
